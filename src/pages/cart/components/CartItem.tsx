    ...
    <div className="product flex flex-col gap-3 md:flex-row md:items-center border-b border-gray-300 py-5 first:pt-0 last:border-b-0">
      ...
      <div className="content flex justify-between items-center flex-1">
        <div className="caption">
          <span className="text-gray-400 dark:text-gray-100 text-sm mb-2 block"> {product.category.name} </span>
          ...
          <div className="star bg-[#fafafa] dark:bg-black rounded-3xl py-1 px-2 w-fit flex items-center gap-1">
            ...
            <Star className='text-yellow-500 dark:text-gray-100 size-4' />
          </div>
        </div>
        <div className="text-right">
          <span className="text-xl dark:text-gray-100"> {price} EGP</span>
          <div className="flex items-center gap-2 mt-3">
            <span onClick={() => removeCartItem(product.id)} className="h-10 px-3 text-primary dark:text-gray-100 flex justify-center items-center cursor-pointer border border-primary rounded-xl transition-all duration-300 hover:bg-primary hover:text-white">
            ...
            <div className="btns overflow-hidden flex items-center justify-around text-text w-[120px] h-10 border border-primary rounded-xl">
              <button onClick={() => updateQuantity(product.id, count - 1)} className="w-[30%] p-2 h-full transition-all duration-300 hover:bg-primary hover:text-white">
                <Minus className='size-4 dark:text-gray-100' />
              </button>
              <span className="w-[40%] h-full flex justify-center items-center dark:text-gray-100"> {count} </span>
              <button onClick={() => updateQuantity(product.id, count + 1)} className="w-[30%] p-2 h-full transition-all duration-300 hover:bg-primary hover:text-white">
                <Plus className='size-4 dark:text-gray-100' />
              </button>
            </div>
          </div>
        </div>
